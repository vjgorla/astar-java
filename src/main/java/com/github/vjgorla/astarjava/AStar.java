...
/**
 * Implementation of A* algorithm for path finding. For more information see
 * <link>http://www.policyalmanac.org/games/aStarTutorial.htm</link>.
 *
 * @author Vijay Gorla
 */
...
public class AStar implements Iterable<Node> {
    ...
    /**
     * Array of all nodes in the grid. First dimension is the width and the second is height.
     */
    ...
    public void setDestination(int x, int y) {
        ...
        for (int i = 0; i < gridWidth; i++) {
            for (int j = 0; j < gridHeight; j++) {
                ...
                nodes[i][j].setH(
                        (Math.abs(destination.getX() - nodes[i][j].getX()) + Math.abs(destination.getY() - nodes[i][j].getY()))
                                * 10);
        ...
        for (int i = (node.getX() - 1); i <= (node.getX() + 1); i++) {
            for (int j = (node.getY() - 1); j <= (node.getY() + 1); j++) {
                if (i >= 0 && i < gridWidth && j >= 0 && j < gridHeight) {
                    ...
                    if (checkNode.isWalkable() && !isAroundCorner(checkNode, node) && !closedList.contains(checkNode)) {
                        if (!openList.contains(checkNode)) {
                            ...
                            // node is better, using G cost as the measure. A lower G cost means
                            // that
    ...
    private boolean isAdjacent(Node node, Node potentialParent) {
        return node.getX() == potentialParent.getX() || node.getY() == potentialParent.getY();
    }

    /**
     * Calculate the movement cost to move from the <tt>potentialParent</tt> to the given
     * <tt>node</tt>, including the cost to get to the <tt>potentialParent</tt>.
     */
